...
def _check_area(area):
    ...
    """
    x1, x2, y1, y2 = area
    assert x1 <= x2, "Invalid area dimensions {}, {}. x1 must be < x2.".format(x1, x2)
    assert y1 <= y2, "Invalid area dimensions {}, {}. y1 must be < y2.".format(y1, y2)
...
def create_plc():
    ...
    landfill = mt.createCube(
        size=[widthlandfill, widthlandfill, Hlandfill],
        pos=[width / 2, width / 2, -Hlandfill / 2],
        marker=3,
    )

    ano = mt.createCube(
        size=[widthAno, ticknessAno, HAno],
...
def create_scheme_Green(world, landfill, ano, plc):
    ...
    # add MALM electrodes
    EAp = [ano.xmax() / 2 + ano.xmin() / 2, ano.ymax() / 2 + ano.ymin() / 2, depthAno]
    ...
    rectangle = plt.Rectangle(
        (width / 2 - widthlandfill / 2, width / 2 - widthlandfill / 2),
        widthlandfill,
        widthlandfill,
        ec="red",
    )
...
def run_fwd_model_Green(malmGreenScheme, mesh):
    ...
    u_elecs = simulateERT(
        mesh,
        scheme=malmGreenScheme,
        res=[[1, SoilR], [2, linerR], [3, SoilR], [4, AnoR]],
        calcOnly=False,
        verbose=True,
        sr=False,
