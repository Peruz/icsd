...
def test_plot3d(file="/data/Landfill_3d/"):
    ...
    mpl_plot.showObs2d(str(MODULE_DIR) + file, coords_elecs, filename="solution.data")

    icsd3d_landfill = iCSD3d(dirName=str(MODULE_DIR) + file)
    icsd3d_landfill.regMesh = "strc"
    icsd3d_landfill.type = "3d"
    icsd3d_landfill.obs_err = (
        "sqrt"  # choose between constant weight and w = 1/sqrt(abs(obs))
    )
    ...
    ax = fig.gca()
    icsd3d_landfill.showResults(ax=ax, mesh="fwd_mesh_landfill.vtk")
    plt.show()

    fig = plt.figure(projection="3d")
    ax = fig.gca()
    icsd3d_landfill.showResults(ax=ax, mesh="fwd_mesh_landfill.vtk", gif3d=True)
    plt.show()
