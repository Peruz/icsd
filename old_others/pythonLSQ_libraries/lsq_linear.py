...
def lsq_linear(A, b, bounds=(-np.inf, np.inf), method='trf', tol=1e-10,
    ...
    # np.all((x_lsq >= lb) & (x_lsq <= up))
    # it is False if at least one value is outside its boundaries
    # if the unbounded solution is valid, enter the clause and accept the unbouded solution as valid
    # the solution respects the boundaries without the need to apply them
        r = A.dot(x_lsq) - b
        cost = 0.5 * np.dot(r, r)
        termination_status = 3 # the unconstrained solution is optimal
        termination_message = TERMINATION_MESSAGES[termination_status]
        g = compute_grad(A, r)
        # Compute gradient of the least-squares cost fucntion.
        # It is a column with the same length of x.
        # Each entry is related to the corresponding variable in X.
        # Each entry depends on how the misfit vector combines with the A row associated with that variable.
        # If the misfit is high where a variable has significant importance,
        # that variable will significantly contribute to the gradient and possibly choosen for the next iteration investigation.
    ...
    # if the unconstrained solution does not repsect the boundaries
    # call the solver that accounts for the boundaries
    # pass them parameters and x_lsq (unconstrined solution)
    if method == 'trf': # default method, call the linear version
