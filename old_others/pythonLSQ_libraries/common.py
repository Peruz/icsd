...
def reflective_transformation(y, lb, ub):
    ...
    if in_bounds(y, lb, ub): # if x_lsq is already inside the boundaries
        return y, np.ones_like(y)

    # make two vectors, they define whethher or not the oboundaries for that parameter are specificied
    # if they are not, the the defalut value is +/- inf, these give False entries   
    lb_finite = np.isfinite(lb)
    ub_finite = np.isfinite(ub)

    x = y.copy()
    g_negative = np.zeros_like(y, dtype=bool) # a all-False vector (0 gives False)

    # only lower boundary, note that x values could also be ok, higher than the boundary
    mask = lb_finite & ~ub_finite # a mask that is True only where the only the lower boundary is given
    # if lb = 0, take y if it is positive or -y (which is postive) if it is negative (i.e., reflect it inside)
    # in general 2*lb - y >= y when y <= lb; i.e., if y is lower than lb, then take s*lb - y
    x[mask] = np.maximum(y[mask], 2 * lb[mask] - y[mask])
    # keep it False if y is lower than its lb
    g_negative[mask] = y[mask] < lb[mask]

    # as above, here only upper bounded
    mask = ~lb_finite & ub_finite 
    x[mask] = np.minimum(y[mask], 2 * ub[mask] - y[mask])
    g_negative[mask] = y[mask] > ub[mask]

    # when both lower and upper boundary are fixed
    mask = lb_finite & ub_finite
    d = ub - lb
    t = np.remainder(y[mask] - lb[mask], 2 * d[mask]) # returns element-wise the division remainder
